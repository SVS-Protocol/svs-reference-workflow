...
export async function runReferenceAuthorization({
  ...
  let rejection;
  try {
    await requireAuthorizedAction({ ...common, action: `${manifest.action.txType}-mismatch-probe` });
    throw new Error("Authorization mismatch probe was unexpectedly accepted.");
  } catch (error) {
    if (error?.message === "Authorization mismatch probe was unexpectedly accepted.") throw error;
    ...
    if (
      ...
      throw new Error(`Authorization mismatch probe did not produce an action-bound SVS rejection: ${rejection.errorName}.`);
    }
  }
...
export function createCaseStudyMarkdown(result) {
  ...
  return `# ${result.workflow.agentName} x ${result.workflow.relyingPartyName}: SVS Reference Workflow\n\n` +
    ...
    `## Verified outcome\n\n` +
    `The exact serialized transaction passed SVS authorization before broadcast. After execution, the same action passed certification, proof freshness, execution binding, custom receipt-registry, independent verification, and official identity checks. Deliberately altered action types were rejected in both phases.\n\n` +
